...
class Keypad:
    def __init__(self):
        self.buttons = []
        self.buttons.append({'text': '1', 'rect': (
            680, 45, 30, 30), 'color': (66, 134, 244), 'textPos': (692, 50)})
        self.buttons.append({'text': '2', 'rect': (
            720, 45, 30, 30), 'color': (66, 134, 244), 'textPos': (732, 50)})
        self.buttons.append({'text': '3', 'rect': (
            760, 45, 30, 30), 'color': (66, 134, 244), 'textPos': (772, 50)})
        self.buttons.append({'text': 'A', 'rect': (
            800, 45, 30, 30), 'color': (239, 36, 14), 'textPos': (812, 50)})
        self.buttons.append({'text': '4', 'rect': (
            680, 85, 30, 30), 'color': (66, 134, 244), 'textPos': (692, 90)})
        self.buttons.append({'text': '5', 'rect': (
            720, 85, 30, 30), 'color': (66, 134, 244), 'textPos': (732, 90)})
        self.buttons.append({'text': '6', 'rect': (
            760, 85, 30, 30), 'color': (66, 134, 244), 'textPos': (772, 90)})
        self.buttons.append({'text': 'B', 'rect': (
            800, 85, 30, 30), 'color': (239, 36, 14), 'textPos': (812, 90)})
        self.buttons.append({'text': '7', 'rect': (
            680, 125, 30, 30), 'color': (66, 134, 244), 'textPos': (692, 130)})
        self.buttons.append({'text': '8', 'rect': (
            720, 125, 30, 30), 'color': (66, 134, 244), 'textPos': (732, 130)})
        self.buttons.append({'text': '9', 'rect': (
            760, 125, 30, 30), 'color': (66, 134, 244), 'textPos': (772, 130)})
        self.buttons.append({'text': 'C', 'rect': (
            800, 125, 30, 30), 'color': (239, 36, 14), 'textPos': (812, 130)})
        self.buttons.append({'text': '*', 'rect': (680, 165, 30, 30),
                             'color': (239, 36, 14), 'textPos': (692, 170)})
        self.buttons.append({'text': '0', 'rect': (
            720, 165, 30, 30), 'color': (66, 134, 244), 'textPos': (732, 170)})
        self.buttons.append({'text': '#', 'rect': (
            760, 165, 30, 30), 'color': (239, 36, 14), 'textPos': (772, 170)})
        self.buttons.append({'text': 'D', 'rect': (
            800, 165, 30, 30), 'color': (239, 36, 14), 'textPos': (812, 170)})
...
class GUI:
    def __init__(self, plant=None, controller=None, monitor=None):
        ...
        # Create components
        self.__icons = {}
        self.__icons["a"] = VesselIcon(
            self.__screen, 100, 50, self.__plant._vessels["a"])
        self.__icons["b"] = VesselIcon(
            self.__screen, 300, 50, self.__plant._vessels["b"])
        self.__icons["mix"] = VesselIcon(
            self.__screen, 200, 125, self.__plant._vessels["mix"])
        self.__icons["heater"] = HeaterIcon(
            self.__screen, 200, 185, self.__plant._lemonator.heater.get)
        self.__icons["temperature"] = SensorIcon(
            self.__screen, 275, 125, self.__plant._lemonator.temperature.read_mc, "°C",  "Temperature")
        self.__icons["distance"] = SensorIcon(
            self.__screen, 275, 150, self.__controller.distance_filter, "mm", "distance")
    ...
    def drawButtons(self) -> None:
        ...
        if self.__run:
            ...
            pygame.draw.polygon(self.__screen, (240, 240, 240), [
                                (527, 15), (527, 35), (543, 25)])
        pygame.draw.rect(self.__screen, (240, 120, 0), [570, 10, 60, 30])
        pygame.draw.polygon(self.__screen, (240, 240, 240), [
                            (585, 15), (585, 35), (600, 25)])
        ...
        for keyPadButton in self.__keypad.getButtons():
            pygame.draw.rect(
                self.__screen, keyPadButton['color'], keyPadButton['rect'])
            label = self.__font.render(
                keyPadButton['text'], False, (255, 255, 255))
            self.__screen.blit(label, keyPadButton['textPos'])

        # LCD LED
        pygame.draw.rect(self.__screen, (52, 191, 30),
                         [680, 215, 6 * 20, 12 * 4])
        ...
        water_pump = "water pump: " + \
            ("on " if self.__plant._lemonator.water_pump.get() else "off")
        ...
        sirup_pump = "sirup pump: " + \
            ("on " if self.__plant._lemonator.sirup_pump.get() else "off")
        ...
        heater = "Heater: " + \
            ("on " if self.__plant._lemonator.heater.get() else "off")
        ...
        self.add_cup = pygame.draw.rect(
            self.__screen, (240, 120, 0), [160, 0, 130, 30])
        self.__screen.blit(
            self.__font.render("%s" % ("Remove cup" if self.__plant._lemonator.reflex.get(
            ) else "Insert cup"), False, (240, 240, 240)),
            [200, 0])

        pygame.draw.circle(self.__screen, (0, 255, 0), (226, 90), 7, 0) if self.__plant._lemonator.led_green.get(
        ) else pygame.draw.circle(self.__screen, (0, 0, 0), (226, 90), 7, 0)

        pygame.draw.circle(self.__screen, (255, 255, 0), (226, 70), 7, 0) if self.__plant._lemonator.led_yellow.get(
        ) else pygame.draw.circle(self.__screen, (0, 0, 0), (226, 70), 7, 0)
    ...
    def drawGraphs(self) -> None:
        ...
        pygame.draw.line(self.__screen, (240, 120, 0), [
                         x0, y0 - (levelSetPoint * scale)], [x0 + 170, y0 - levelSetPoint * scale])
        ...
        for reading in self.__monitor._sensorReadings["distance"][-170:]:
            ...
            x += 1
            y = y0 - (reading * scale)
            pygame.draw.line(self.__screen, (0, 240, 0),
                             [prevX, prevY], [x, y], 2)
        x, y = x0, y0
    ...
    def update(self) -> None:
        ...
        for (name, icon) in self.__icons.items():
            if not (name == 'mix' and not self.__plant._lemonator.reflex.get()):
                icon.draw()
        self.drawPipes()
                ...
                if event.type == QUIT:
                    ...
                    if 500 < pos[0] <= 630:
                        if 10 < pos[1] <= 40 and pos[0] < 560:
                            ...
                            if self.__plant._lemonator.water_pump.get():
                                self.__plant._lemonator.water_pump.set(0)
                            else:
                                self.__plant._lemonator.water_pump.set(1)
                        elif 90 < pos[1] <= 120:
                            if self.__plant._lemonator.sirup_pump.get():
                                self.__plant._lemonator.sirup_pump.set(0)
                            else:
                                self.__plant._lemonator.sirup_pump.set(1)
                        elif 130 < pos[1] <= 160:
                            if self.__plant._lemonator.heater.get():
                                self.__plant._lemonator.heater.set(0)
                            else:
                                self.__plant._lemonator.heater.set(1)
                        elif 170 < pos[1] <= 200:
                            self.__tap = False if self.__tap else True
                    # keypad
                    for keypadButton in self.__keypad.getButtons():
                        if pygame.Rect(keypadButton['rect']).collidepoint(pos):
                            self.__plant._lemonator.keypad.putc(
                                keypadButton['text'])
...
class VesselIcon(Icon):
    ...
    def draw(self) -> None:
        ...
        distance = (50 / 100) * \
            (self._vessel.getFluidAmount() / (max_liquid / 100))
        pygame.draw.rect(self._screen, (color * 2.55, 0, 255 - color * 2.55),
                         [self._x, self._y + 51 - distance, 50, distance])
        pygame.draw.lines(self._screen, (0, 0, 0), False, [(self._x, self._y), (
            self._x, self._y + 50), (self._x + 50, self._y + 50), (self._x + 50, self._y)], 2)
...
class HeaterIcon(Icon):
    ...
    def draw(self) -> None:
        pygame.draw.line(self._screen, (0, 0, 0), [
                         self._x, self._y], [self._x + 50, self._y], 4)
        if self._heater():
            x = self._x + 2
            for i in range(6):
                pygame.draw.lines(self._screen, (0, 0, 0), True, [
                                  (x, self._y - 2), (x + 3, self._y - 6), (x + 6, self._y - 2)], 2)
                x += 8
